...
client.on('messageReactionAdd', async (reaction, user) => {
  ...
  if (
    ...
    let vtumon = null
    let vtumon_list = game.getVtumonOfPlayer(user.id); // <-- Lấy vtumon của người chơi
    ...
    switch (reaction.emoji.name) {
      ...
      case '🍇': //Item List
        ...
        const pollMessage = await reaction.message.channel.send({
          content: `${addTag(user.id)}, bạn chọn sử dụng item nào?${ct}`,
          fetchReply: true
        });
        n = 0;
        for (const i of item_list) {
      ...
      case '🐱':
        ...
        if (vtumon) {
          ...
          // GỬI API với query command , nếu thành công thì thếu thất bại thì 
          await reaction.message.channel.send(`${addTag(user.id)} đã sử dụng ${game.query_command.type} ${game.getNameTypeIdQueryCommand()} với đối tượng vtumon là ${vtumon.name}`);
      ...
      switch (item.name) {
        ...
        case 'Superchat':
          ...
          if (!vtumon_list || vtumon_list.length === 0) {
            await reaction.message.channel.send(`${addTag(user.id)}, bạn không có vtumon nào! 🍇`);
            return; // Không có item nào để chọn
          }
          let ct = '';
          let n = 0;
          ...
          const pollMessage = await reaction.message.channel.send({
            content: `${addTag(user.id)}, Bạn đang sử dụng Item Super Chat, bạn chọn vtumon nào?${ct}`,
            fetchReply: true
          });
          n = 0;
          for (const i of vtumon_list) {
